...
namespace BilgeIK.SERVICE.TwoFactorService
{
    public class EmailSender
    {
        ...
        private async Task Execute(string email, string code)
        {
            ...
            var from = new EmailAddress("trtutkucakir@gmail.com");
            var subject = "İki Adımlı Kimlik Doğrulama Kodu";
            var to = new EmailAddress(email);
            var htmlContent = $"<h2>Siteye giriş yapabilmek için oluşturulan doğrulama kodu aşağıdadır:</h2><h3>Kodunuz: {code}</h3>";
